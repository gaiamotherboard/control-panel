...
class HardwareScanUploadForm(forms.Form):
    """
    Form for uploading lshw JSON hardware scan files.
    Validates file size and JSON format.
    """

    file = forms.FileField(
        label="LSHW JSON File",
        help_text="Upload the output from 'lshw -json' command",
    ...
    def clean_file(self):
        ...
        3. Check it looks like lshw output
        """
        ...
        try:
            ...
            # Basic validation - should be a dict with some expected lshw structure
            if not isinstance(data, dict):
                raise ValidationError("Invalid lshw format: expected JSON object.")
